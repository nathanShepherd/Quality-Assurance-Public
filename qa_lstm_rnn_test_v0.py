...
all_incidents = all_incidents[:3000]
        ...
        if len(inc.lexicon) <= x_length:
            ...
            for i in range(len_append):
                ...
                try:
                    ...
                    padding.append(np.zeros(len(model['a'])))
...
class NeuralComputer:
    def __init__(self, x, y, hidden_size=100):
        self.x = x
        self.y = y
    ...
    def Perceptron(self, tensor):
        ...
        #Shape is equal to hidden_size
        print("Shape of RNN: Final State", shape[-1])
        
        V0 = tf.Variable(tf.truncated_normal([shape[-1], 100]))
        b0 = tf.Variable(tf.truncated_normal([100]))
        l0 = tf.sigmoid(tf.matmul(tensor, V0) + b0)
        '''
        V1 = tf.Variable(tf.truncated_normal([100, 75]))
        b1 = tf.Variable(tf.truncated_normal([75]))
        l1 = tf.sigmoid(tf.matmul(l0, V1) + b1)
        
        V2 = tf.Variable(tf.truncated_normal([75, 50]))
        b2 = tf.Variable(tf.truncated_normal([50]))
        l2 = tf.sigmoid(tf.matmul(l1, V2) + b2)
        '''
        V3 = tf.Variable(tf.truncated_normal([100, 50]))
        b3 = tf.Variable(tf.truncated_normal([50]))
        l3 = tf.sigmoid(tf.matmul(l0, V3) + b3)
        
        weights = tf.Variable( tf.zeros([50, self.out_dim]),name='weights')
        biases = tf.Variable(tf.zeros([self.out_dim]),name='biases')

        logits = tf.nn.softmax(tf.matmul(l3, weights) + biases)
    ...
    def init_placeholders(self, n_classes, batch_size):
        ...
        x = tf.placeholder(tf.float32, shape=(None, None, self.in_dim))
        #x = tf.placeholder(tf.float32, shape=(None, None,None))
        y_ = tf.placeholder(tf.float32, shape=(batch_size, n_classes))
        
        lstm_cell_1 = tf.contrib.rnn.LSTMCell(1000, reuse=None)
        lstm_cell_2 = tf.contrib.rnn.LSTMCell(500, reuse=None)
        lstm_cell_3 = tf.contrib.rnn.LSTMCell(self.hidden_size, reuse=None)

        # Encapsulate layers of layers into a multilayer cell
        multi_lstm_cells = tf.contrib.rnn.MultiRNNCell(cells=[lstm_cell_1,
                                                              lstm_cell_2,
                                                              lstm_cell_3],
                                                       state_is_tuple=True)
        # get final state from encapsulated cells
        _, final_state = tf.nn.dynamic_rnn(multi_lstm_cells, x, dtype=tf.float32)

        # Feed RNN output into multi-layer perception to classify incidents
        y, W, b = self.Perceptron(final_state[-1][-1])
    ...
    def train(self, test_x, in_str, batch_size=1000, training_epochs=10,learning_rate=.5,display_step=1, reuse=False):
    ...
    def max_of_predictions(self, predictions):
        out_arr = []
        out_acc = []#Z-score for all_predictions
        printed = False
        for pred in predictions:
            #print('\n========')
            _max = [0, 0]# [index, value]
            if not printed:
                print("_+_+_+_+_+_+\nZ-score of first prediction:", end="")
                mean = np.mean(pred[0])
                stddev = np.std(pred[0])
            for vect in pred:
                for i, num in enumerate(vect):
                    if _max[1] <  num:
                        _max[1] = num
                        _max[0] = i
                    #print('{}::{}'.format(i,vect))
...
########## START OF MISSING CATEGORY GRAPH ##########
...
nn.train(cat_input_data,"Missing Category",
         ...
         training_epochs=60, learning_rate=.5,#lr@0.05=best
...
sd_nn = NeuralComputer(sd_train_x, sd_train_y)
...
sd_nn.train(sd_input_data,"Service Detail",
         
         training_epochs=1,learning_rate=.5,
         
         batch_size=100, display_step=3)

elapsed_time = time.time() - start_time

print('Time elapsed:', elapsed_time)

sd_predictions = sd_nn.predict([[[inc.lexicon]] for inc in testing_incs])

sd_predictions = sd_nn.max_of_predictions(sd_predictions)
